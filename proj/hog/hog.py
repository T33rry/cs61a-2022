...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            dice_nums = strategy0(score0, score1)
            score0 = update(dice_nums, score0, score1, dice)
            who += 1
        else:
            dice_nums = strategy1(score1, score0)
            score1 = update(dice_nums, score1, score0, dice)
            who -= 1
    # END PROBLEM 5
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    dice_nums = strategy(0, 0)
    score0 = 0
    while score0 < goal:
        score1 = 0
        while score1 < goal:
            current_dice_nums = strategy(score0, score1)
            if current_dice_nums != dice_nums:
                return False
            score1 +=1
        score0 +=1
...
def make_averaged(original_function, total_samples=1000):
    ...
    def averaged_dice(*args):
        ...
        for i in range(total_samples):
            cur_values = original_function(*args)
            sum += cur_values
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while num_rolls <= 10:
        ...
        outputs_list.append(roll_average(num_rolls, dice))
        num_rolls+=1
    max_item = max(outputs_list)
    return outputs_list.index(max_item) + 1
    # END PROBLEM 9
